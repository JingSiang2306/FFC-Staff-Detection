...
DEFAULT_DATA = PROJECT_ROOT / "data" / "tag_dataset_balanced_v2" / "tag_data.yaml"
DEFAULT_MODEL = PROJECT_ROOT / "yoloModel" / "best_tag_v1.1.pt"
DEFAULT_PROJECT = PROJECT_ROOT / "outputs" / "test_tag_evaluation"


def parse_args():
    """Read staff-tag test-evaluation options from the command line."""
    parser = argparse.ArgumentParser(description="Evaluate the staff-tag detector on the frozen test split.")
    parser.add_argument("--model", type=Path, default=DEFAULT_MODEL, help="Path to the tag model best.pt")
    ...
    parser.add_argument("--batch", type=int, default=16, help="Test batch size")
    parser.add_argument("--workers", type=int, default=4, help="Data-loading worker processes")
    parser.add_argument("--name", default="yolo26n_staff_tag_v2_test", help="Output run name")
...
def read_dataset_names(data_path):
    """Read and normalize class names from tag_data.yaml."""
    with data_path.open("r", encoding="utf-8") as file:
        data = yaml.safe_load(file) or {}

    if not data.get("test"):
        raise ValueError("tag_data.yaml does not define a test split.")
    ...
    if isinstance(names, list):
        ...
        raise ValueError("tag_data.yaml does not contain valid class names.")

    return names


def write_metrics_csv(output_path, model_path, imgsz, metrics):
    """Save the main staff-tag test metrics in one CSV row."""
    row = {
        "split": "test",
...
def main():
    ...
    print("Evaluating the frozen staff-tag test split. Do not tune the model using this result.")
    metrics = model.val(
        data=str(data_path),
        split="test",
        device=device,
    ...
    print("\nStaff-tag test evaluation completed successfully")
    print(f"Precision:  {metrics.box.mp:.4f}")
    print(f"Recall:     {metrics.box.mr:.4f}")
    print(f"mAP50:      {metrics.box.map50:.4f}")
    print(f"mAP50-95:   {metrics.box.map:.4f}")
    print(f"Results:    {run_path}")
    print("Note: the test split has only 5 positive images, so one miss changes recall by 20 percentage points.")
