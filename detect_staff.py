...
def parse_args():
    ...
    parser.add_argument("--conf", type=float, default=0.25, help="Person confidence floor")
    parser.add_argument("--imgsz", type=int, choices=(640, 960, 1280), default=640)
    parser.add_argument("--tag-conf", type=float, default=0.3)
...
def validate_args(args):
    ...
    if args.vote_window <= 0 or args.vote_min <= 0:
...
def draw_staff_box(frame, box, track_id):
...
def main():
    ...
    positive_frames = defaultdict(deque)
    confirmed_staff = set()
    ...
    try:
        while True:
            ...
            oldest_allowed = frame_count - args.vote_window + 1
            for (box, track_id), tag_score in zip(tracked_people, tag_scores):
                history = positive_frames[track_id]
                while history and history[0] < oldest_allowed:
                    history.popleft()
                if tag_score >= args.tag_conf:
                    history.append(frame_count)
                    tag_positive_events += 1
                if len(history) >= args.vote_min:
                    confirmed_staff.add(track_id)
                if track_id in confirmed_staff:
                    draw_staff_box(annotated_frame, box, track_id)
    ...
    print(f"Confirmed staff track IDs: {sorted(confirmed_staff)}")
